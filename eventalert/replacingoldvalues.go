...
// ReplacingOldValues заменяет старые значения структуры TypeEventForAlert
// новыми значениями. Изменяемые поля:
// CommonEventType
// Details - детальная информация о событии
// Object - объект события
func (e *TypeEventForAlert) ReplacingOldValues(element TypeEventForAlert) (int, error) {
	var (
		err                  error
